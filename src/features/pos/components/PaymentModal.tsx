...
export const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  return (
    <div className="min-h-screen bg-bg-light text-gray-800 font-sans flex flex-col">
      {/* Nav */}
      <nav className="bg-white border-b border-primary/20 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
            <span className="material-icons text-secondary">arrow_back</span>
          </button>
          <div>
            <h1 className="text-xl font-bold tracking-tight">Payment - {tableName}</h1>
            <p className="text-sm text-gray-500">Order #{order.id.slice(-4)}</p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-xs uppercase tracking-wider text-gray-400 font-semibold">Total Remaining</p>
          <p className="text-xl font-bold text-gray-800">${remainingTotal.toFixed(2)}</p>
        </div>
      </nav>

      <main className="flex-1 max-w-7xl mx-auto w-full p-6 grid grid-cols-1 md:grid-cols-2 gap-8 h-[calc(100vh-88px)]">
        {/* Left: Unpaid Items */}
        <section className="flex flex-col h-full bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            ...
            <span className="bg-gray-200 text-gray-600 text-xs font-bold px-2 py-1 rounded-full">{leftItems.length} Left</span>
          </div>
        ...
        {/* Right: Items to Pay */}
        <section className="flex flex-col h-full bg-white rounded-xl border-2 border-primary/20 shadow-lg shadow-primary/5 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {rightItems.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-300">
                ...
                <p className="text-sm">Select items from the left to pay</p>
              </div>
            ) : (
          ...
          <div className="p-6 bg-gray-50 border-t border-gray-100">
            <div className="flex justify-between items-center mb-6">
              ...
              <span className="text-3xl font-black text-primary">${payTotal.toFixed(2)}</span>
            </div>
            <button
              onClick={() => onProcessPayment(rightItems)}
              disabled={rightItems.length === 0}
              className="w-full bg-primary hover:bg-green-400 text-white font-black text-xl py-4 rounded-xl transition-all shadow-lg shadow-primary/30 flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
            >
